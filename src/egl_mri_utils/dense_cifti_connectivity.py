...
def dcov_from_dtseries_list_lowmem(input_paths, output_dconn_path, volumes_to_drop=None, chunk_size=1000):
    """
    Calculates a dense covariance matrix (saved as dconn) from a series of dtseries files
    using a low-memory chunked approach, removing specific volumes. Scrubbing is allowed
    with the volumes_to_drop command.
    
    Prior to concatenation all greyordinate timeseries within a run will be demeaned
    so that each timepoint contributes equally to the final correlation estimates.
    ...
    for i, temp_input in enumerate(input_paths):
        ...
        # Subtract mean
        data_centered = data_sliced - data_sliced.mean(axis=0)
        data_centered_list.append(data_centered)
    ...
    data_concat = np.vstack(data_centered_list)
    num_total_vols = data_concat.shape[0]
    print("   Concatenated time series has {} volumes.".format(num_total_vols))
    del data_centered_list # free memory
    ...
    # 4. Calculate Correlation in Chunks
    print(f"Computing covariance in chunks of {chunk_size} rows...")
    
    for i in range(0, n_greyordinates, chunk_size):
        end_idx = min(i + chunk_size, n_greyordinates)
        
        # Get chunk (Time x Chunk)
        chunk_data = data_concat[:, i:end_idx]
        
        # Matrix Multiply: (Chunk_T) dot (All_Data)
        chunk_cov = np.dot(chunk_data.T, data_concat)/num_total_vols
        
        # Write to disk
        dconn_memmap[i:end_idx, :] = chunk_cov
